# -*- Python -*-
# $id: scsi_disk.py,v 1.4 2004/07/21 04:02:44 benj Exp $
...
# --- edited for SCSI Kyle L. // kysle@hotmail.com // 08/04/05
...
def list_block_controllers():
    ...
    @returns: short names of all controllers of block devices in the
        system which Linux knows about, e.g. ['sda', 'sdb']
    @returntype: list of strings
    """
    block = False
...
#    f.close()
#    o.close()
    return {'serialNo': names.groups()[0],
            'model': names.groups()[2]}

#    _scsi_names_re = re.compile("Vendor:")
#    names = filter(_scsi_names_re.search, open("/var/log/dmesg").readlines())
#	_scsi_blah = re.blah("foo(bar)(baz)")
#	match = _scsi_names_re.search(thing).groups()[0] is "bar"
#	match = _scsi_names_re.search(thing).groups()[1] is "baz"
#    return names


def supported_modes(blockDevice):
    """Return the supported PIO and DMA modes for a SCSI device.
...
# Parse stuff from scisi.c:scsi_dump_status()
...
_scsi_dump = re.compile(r'(?P<drive>%s)' %
                      string.join(map(lambda s: '(?:%s)' % (s,), ['sda', 'sdb'] or drives), '|')
                      + r': (?P<msg>.*?): ')

_scsi_status_dump = re.compile(r'status=(?P<statcode>0x..) '
                             r'{ (?P<statwords>.*?) }$')

_scsi_error_dump = re.compile(r'error=(?P<errcode>0x..) { (?P<errwords>.*?) }')
...
def take_syslog_line(line):
    ...
    if match is None:
        return None
    scsi_match = _scsi_dump.match(line, match.end())
    if scsi_match is not None:
        match = scsi_match
        msg.update(match.groupdict())

        status_match = _scsi_status_dump.match(line, match.end())
        if status_match is not None:
            msg.update(status_match.groupdict())
            msg['statwords'] = string.split(msg['statwords'])
            return msg

        error_match = _scsi_error_dump.match(line, match.end())
...
# byte scsi_dump_status (scsi_drive_t *drive, const char *msg, byte stat)
# {
...
#				byte cur = IN_BYTE(SCSI_SELECT_REG);
#				if (cur & 0x40) {	/* using LBA? */
#					printk(", LBAsect=%ld", (unsigned long)
#					 ((cur&0xf)<<24)
#					 |(IN_BYTE(SCSI_HCYL_REG)<<16)
#					 |(IN_BYTE(SCSI_LCYL_REG)<<8)
#					 | IN_BYTE(SCSI_SECTOR_REG));
#				} else {
#					printk(", CHS=%d/%d/%d",
#					 (IN_BYTE(SCSI_HCYL_REG)<<8) +
#					  IN_BYTE(SCSI_LCYL_REG),
#					  cur & 0xf,
#					  IN_BYTE(SCSI_SECTOR_REG));
#				}
